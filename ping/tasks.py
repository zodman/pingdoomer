...
def fetch_accounts():
    ...
    for i in resp:
        external_id = i.get("external_id")
        result.append(f"account_{external_id}")
    return result
...
def schedule_alerts():
    """
    SELECT mean("return_code") AS "mean_return_code" FROM "ping"."autogen"."account_1069"  WHERE time > now() - 5m AND time < now()   GROUP BY time(10m), "hostname" FILL(null)
    """
